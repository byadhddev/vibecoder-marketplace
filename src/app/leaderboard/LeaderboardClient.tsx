...
export default function LeaderboardClient({ initialBuilders }: {
                ...
                return (
                    <div className={`${tile.colSpan} flex flex-col items-center justify-center gap-2 p-4 min-h-[120px] transition-all duration-300`} style={{ background: bg }}>
                        ...
                        <button onClick={() => setSortBy('views')}
                            className={`text-[9px] font-mono uppercase tracking-[0.15em] transition-colors ${sortBy === 'views' ? 'text-brand-red font-bold' : 'text-[#9b9a97] hover:text-[#37352f]'}`}>
                            By Views
                        </button>
                    </div>
                );
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-2 text-[#9b9a97]">
            ...
            <span className="text-[#37352f] font-medium">leaderboard</span>
        </div>
    );
