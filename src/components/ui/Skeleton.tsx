...
function Shimmer({ className = '' }: { className?: string }) {
    return (
        <div className={`animate-pulse bg-vc-skeleton rounded ${className}`} />
    );
}
...
function Avatar({ size = 24 }: { size?: number }) {
    return <div className="animate-pulse bg-vc-skeleton rounded-sm" style={{ width: size, height: size }} />;
}
...
function Tile({ span = 2, minH = '140px', lines = 3, dark = false }: {
    ...
    const bg = dark ? 'bg-vc-dark' : 'bg-vc-surface';
    const shimBg = dark ? 'bg-[var(--vc-skeleton-dark)]' : '';
    return (
        <div className={`${colClass} p-5 md:p-6 flex flex-col justify-between ${bg}`} style={{ minHeight: minH }}>
            <div className="flex items-center gap-3">
                <Shimmer className={`w-5 h-5 rounded-sm ${shimBg}`} />
                <Shimmer className={`h-3.5 w-24 ${shimBg}`} />
                <div className="flex-1" />
                <Shimmer className={`h-2 w-12 ${shimBg}`} />
            </div>
            <div className="space-y-2 mt-auto">
                {Array.from({ length: lines }).map((_, i) => (
                    <Shimmer key={i} className={`h-2.5 ${i === 0 ? 'w-3/4' : i === 1 ? 'w-1/2' : 'w-1/3'} ${shimBg}`} />
                ))}
            </div>
        </div>
    );
...
function ArtodeTile() {
    return (
        <div className="col-span-1 aspect-square bg-vc-dark flex items-center justify-center">
            <div className="w-4 h-4 rounded-sm bg-[var(--vc-skeleton-dark)] animate-pulse" />
        </div>
    );
}
...
function TitleTile({ span = 2 }: { span?: 2 | 3 }) {
    ...
    return (
        <div className={`${colClass} p-6 md:p-8 bg-vc-dark flex flex-col justify-end min-h-[120px]`}>
            <Shimmer className="h-5 w-2/3 bg-[var(--vc-skeleton-dark)] mb-2" />
            <Shimmer className="h-2.5 w-1/3 bg-[var(--vc-skeleton-dark)]" />
        </div>
    );
}
...
function CounterTile() {
    return (
        <div className="col-span-1 p-5 md:p-6 bg-vc-surface flex flex-col justify-between min-h-[120px]">
...
function FilterBar() {
    return (
        <div className="col-span-2 md:col-span-4 p-4 md:p-5 bg-vc-surface flex items-center gap-3 min-h-[60px]">
...
function SectionHeader({ width = 'w-32' }: { width?: string }) {
    return (
        <div className="col-span-2 md:col-span-4 px-5 py-3 bg-vc-surface-raised">
...
function SignatureTile() {
    return (
        <div className="col-span-2 md:col-span-2 p-5 md:p-6 bg-vc-surface flex flex-col justify-center min-h-[80px]">
            <div className="h-px bg-vc-border mb-3" />
...
function AvatarTile() {
    return (
        <div className="col-span-1 aspect-square bg-vc-skeleton animate-pulse" />
    );
}
